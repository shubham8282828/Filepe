...
# Must set event loop BEFORE importing pyrogram (Python 3.14 fix)
try:
...
async def handle_upload(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if file_size > MAX_TG_BOT_SIZE and pyro_available:
            # Large file — use Pyrogram (2GB support)
            await processing_msg.edit_text(
                f"⬇️ Downloading {size_mb:.1f} MB via Pyrogram...",
            )
            file_bytes = await pyro_download_file(
                tg_file.file_id, file_name
            )
            if not file_bytes:
                # Pyrogram failed — try Bot API as fallback
                ...
                try:
                    ...
                    await processing_msg.edit_text(
                        "❌ <b>Download failed!</b>\n\n"
                        "Pyrogram unavailable & file too large for Bot API.\n"
                        f"Please use: <code>/addfile PIXELDRAIN_ID {file_name}</code>",
                        parse_mode="HTML"
                    )
                    return
        else:
            # Small file — use Bot API (fast)
            file_obj = await ctx.bot.get_file(tg_file.file_id)
            file_bytes = bytes(await file_obj.download_as_bytearray())
