...
def build_application() -> Application:
    ...
    # TG CDN upload handler (admin) — uses Telegram as storage, no Pixeldrain
    # Priority: TG CDN first (if STORAGE_CHANNEL_ID set), else Pixeldrain
    if STORAGE_CHANNEL_ID:
        app.add_handler(MessageHandler(
            filters.User(ADMIN_IDS) & (filters.VIDEO | filters.Document.ALL | filters.AUDIO) & ~filters.FORWARDED,
            handle_tg_upload   # ← New: Telegram CDN storage
        ))
        app.add_handler(MessageHandler(
            filters.User(ADMIN_IDS) & (filters.VIDEO | filters.Document.ALL) & filters.FORWARDED,
            handle_tg_upload   # ← Forward from channel also saved to CDN
        ))
    else:
        # Fallback: old Pixeldrain-based system
        app.add_handler(MessageHandler(
            filters.User(ADMIN_IDS) & (filters.VIDEO | filters.Document.ALL | filters.AUDIO) & ~filters.FORWARDED,
            handle_upload
        ))
