...
def stream():
    ...
    try:
        ...
        def generate():
            for chunk in pd_resp.iter_content(chunk_size=512 * 1024):
                if chunk:
                    yield chunk
...
async def run_bot():
    """Run the Telegram bot asynchronously — Python 3.14 compatible."""
    max_retries = 10
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            logger.info(f"✅ Bot started. Polling for updates... (attempt {attempt})")
            ...
            if "Conflict" in err_str:
                logger.warning(f"⚠️ Conflict detected (another instance running). Retrying in {retry_delay}s... [{attempt}/{max_retries}]")
                await asyncio.sleep(retry_delay)
                retry_delay = min(retry_delay + 5, 60)  # backoff up to 60s
            else:
...
def main():
    ...
    # Start Flask in background thread (daemon so it dies with main process)
    flask_thread = threading.Thread(target=run_flask, daemon=True)
    flask_thread.start()
    logger.info(f"✅ Flask streaming server started on port {PORT}")

    # Explicitly create new event loop — fixes Python 3.14 asyncio issue
