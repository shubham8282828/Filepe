...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if payload.startswith("file_"):
        # NEVER redirect file requests — always serve on current bot
        # User may have gotten this bot's link specifically
        get_user(user.id)
        track_active_user(user.id)
...
async def handle_upload(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Generate links for ALL bots so any bot can serve this file
        all_links = []

        # Always include current bot
        current_link = f"https://t.me/{BOT_USERNAME}?start=file_{file_unique_id}"
        all_links.append(f"🤖 @{BOT_USERNAME}:\n<code>{current_link}</code>")

        # Add links for other bots from ALL_BOTS config
        for bot in ALL_BOTS:
            if bot["username"] != BOT_USERNAME:
                link = f"https://t.me/{bot['username']}?start=file_{file_unique_id}"
                all_links.append(f"🤖 @{bot['username']}:\n<code>{link}</code>")

        links_text = "\n\n".join(all_links)

        # Also save universal file_unique_id in DB for cross-bot access
        await processing_msg.edit_text(
            f"✅ <b>File Uploaded Successfully!</b>\n\n"
            f"📁 Name: <code>{file_name}</code>\n\n"
            f"🔗 <b>Share Links (all bots):</b>\n\n"
            f"{links_text}\n\n"
            f"📌 Share any link — works on all bots!",
            parse_mode="HTML"
        )
...
<head>
  ...
  <!-- Plyr Video Player — supports more codecs + mobile -->
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .powered {
      margin-top: 10px; font-size: 0.72rem; color: #444;
    }
    .download-btn {
      margin-top: 14px;
      padding: 10px 24px;
...
<body>
  ...
  <div class="error-box" id="errorBox">
    <p>⚠️ Video cannot play in browser.</p>
    <p style="margin-top:8px;font-size:0.8rem;">Try downloading it instead:</p>
    <a class="download-btn" href="{{ stream_url }}" download="{{ file_name }}">
      ⬇️ Download Video
    </a>
  </div>
  ...
  <script>
    ...
    // Show error box if video fails to load
    const video = document.getElementById('player');
    video.addEventListener('error', function() {
      document.getElementById('errorBox').style.display = 'block';
    });

    // Also show error if no data after 10 seconds
    setTimeout(function() {
...
def stream():
    ...
    pd_url = PD_DOWNLOAD_URL.format(id=pd_id)

    # Forward request to Pixeldrain with proper range support
    try:
        pd_headers = {"Range": range_header}
        if PIXELDRAIN_API_KEY:
            import base64
            creds = base64.b64encode(f":{PIXELDRAIN_API_KEY}".encode()).decode()
            pd_headers["Authorization"] = f"Basic {creds}"
        ...
        # Force correct MIME type based on filename
        raw_ct = pd_resp.headers.get("Content-Type", "")
        ...
        def generate():
            try:
                for chunk in pd_resp.iter_content(chunk_size=256 * 1024):  # 256KB chunks
                    if chunk:
                        yield chunk
            except Exception as ex:
                logger.warning(f"Stream interrupted: {ex}")
            finally:
                pd_resp.close()
        ...
        logger.error(f"Stream error for {pd_id}: {e}")
        abort(502)
