...
async def handle_upload(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if file_size > MAX_TG_BOT_SIZE:
            # LARGE FILE — Stream from Telegram CDN directly to Pixeldrain
            await processing_msg.edit_text(
                f"⬇️ <b>Large file detected!</b>\n"
                f"━━━━━━━━━━━━━━━━━━━━━\n\n"
                f"📦 Size: <b>{size_mb:.1f} MB</b>\n"
                f"🔄 Streaming to cloud storage...\n"
                f"⏳ Please wait (may take 1-5 mins for large files)",
                parse_mode="HTML"
            )

            try:
                # Get Telegram file path (works up to 20MB for path, but URL works for larger)
                # Use bot token to construct direct CDN URL
                tg_file_obj = await ctx.bot.get_file(tg_file.file_id)
                tg_url = tg_file_obj.file_path

                # If it's a full URL, use directly
                if not tg_url.startswith("http"):
                    tg_url = f"https://api.telegram.org/file/bot{BOT_TOKEN}/{tg_url}"

                logger.info(f"Telegram CDN URL obtained: {tg_url[:80]}...")

                # Stream directly from Telegram CDN to Pixeldrain
                pd_id = await asyncio.get_event_loop().run_in_executor(
                    None,
                    lambda: stream_upload_to_pixeldrain(tg_url, file_name)
                )

                if not pd_id:
                    # Try Pyrogram as fallback
                    if pyro_available:
                        await processing_msg.edit_text(
                            f"🔄 Trying alternate method...",
                        )
                        file_bytes = await pyro_download_file(tg_file.file_id, file_name)
                        if file_bytes:
                            pd_id = await asyncio.get_event_loop().run_in_executor(
                                None,
                                lambda: upload_to_pixeldrain(file_bytes, file_name)
                            )
                ...
                if not pd_id:
                    await processing_msg.edit_text(
                        ...
                        "┣ 1️⃣ Go to pixeldrain.com\n"
                        "┣ 2️⃣ Upload your file\n"
                        f"┗ 3️⃣ Use: <code>/addfile PD_ID {file_name}</code>",
                        parse_mode="HTML"
                    )
                    return
                ...
                await processing_msg.edit_text(
                    ...
                    f"Please use: <code>/addfile PD_ID {file_name}</code>",
                    parse_mode="HTML"
                )
                return
        else:
...
def main():
    ...
    # Start Pyrogram client (user account — 2GB file support)
    if PYROGRAM_AVAILABLE and TELEGRAM_SESSION:
        ...
        logger.info("ℹ️ Pyrogram not configured — using Bot API only (20MB limit)")
