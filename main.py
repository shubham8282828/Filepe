...
# Collections
# users collection is per-bot (each bot tracks its own users)
# files, payments, referrals are SHARED across all bots
users_col    = db[f"users_bot{SERVER_ID}"]   # Per-bot: users_bot1, users_bot2, etc.
...
def setup_indexes():
    ...
    logger.info(f"✅ MongoDB indexes created. Users collection: users_bot{SERVER_ID}")
...
def _check_token_db(user_id: int) -> bool:
    """Internal: check DB for token/premium validity."""
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if payload.startswith("file_"):
        # Existing users always served — no redirect
        get_user(user.id)
        track_active_user(user.id)
...
async def send_redirect_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Redirect new user to next bot in chain."""
    user = update.effective_user
    ...
    logger.info(f"Redirected new user {user.id} to {NEXT_BOT_NAME}")
...
def stream():
    ...
    try:
        ...
        content_type   = pd_resp.headers.get("Content-Type", "video/mp4")
